document
  ...
  .addEventListener('submit', function (event) {
    ...
    var passwordValid = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/.test(
      password
    ) // Password should be at least 8 characters long, contain numbers and both lowercase and uppercase letters
    var phoneValid = /^\+?[\d\s-]{7,15}$/.test(phone) // Phone number with optional country code, area code, and standard number format
    ...
    document.getElementById('emailFeedback').style.display = emailValid
      ? 'none'
      : 'block'
    document.getElementById('passwordFeedback').style.display = passwordValid
      ? 'none'
      : 'block'
    document.getElementById('phoneFeedback').style.display = phoneValid
      ? 'none'
      : 'block'
    ...
    document.getElementById('passwordFeedback').textContent = passwordValid
      ? ''
      : 'Password should be at least 8 characters long, contain numbers and both lowercase and uppercase letters.'
    ...
    if (formValid) {
      document.getElementById('registrationFeedback').textContent =
        'Your user registration was accepted!'
